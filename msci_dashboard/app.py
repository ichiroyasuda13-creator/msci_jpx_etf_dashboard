...
def main():
    ...
    try:
        # Check one liquid ticker for timing
        ref_ticker = "2559.T" 
        ref_info = yf.Ticker(ref_ticker).info
        ts = ref_info.get('regularMarketTime')
        if ts:
            last_dt = datetime.fromtimestamp(ts)
            st.caption(f"Data as of: {last_dt.strftime('%Y-%m-%d %H:%M:%S')}")
    except:
